...
describe('Tag e2e test', () => {
  ...
  it('should load create Tag page', async () => {
    ...
    expect(await tagUpdatePage.getPageTitle().getAttribute('id')).to.match(/riverApp.tag.home.createOrEditLabel/);
  });

  it('should create and save Tags', async () => {
    tagUpdatePage.setTagNameInput('tagName');
    expect(await tagUpdatePage.getTagNameInput()).to.match(/tagName/);
